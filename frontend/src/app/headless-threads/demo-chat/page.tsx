 ...
 *
 * The app-wide provider talks to `/api/copilotkit`, which registers 24 agents
 ...
 * it never touches the chat's `threadId`. The chat's own
 * `startNewThread` lives on `CopilotChatConfigurationValue`, which is
 * unavailable here on two counts: this demo mounts no
 * `CopilotChatConfigurationProvider`, and the chat is prop-controlled, which
 * the Lifecycle page says makes those setters no-op and log a warning.
 *
 * So the second step has to be ours, and clearing the prop to `undefined` is
 * NOT enough on its own: with no prop the chat falls through to a minted
 * fallback computed once at mount, so clearing returns to the SAME id every
 * time and the button appears to do nothing. Bumping a React `key` forces the
 * remount that re-runs that memo.
 *
 * The provider-driven equivalent is on /threads-lifecycle, which passes no prop
