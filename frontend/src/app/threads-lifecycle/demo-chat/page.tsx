 ...
 * to sit *inside* the provider — which is also why the chat below it responds
 * to the buttons without being passed anything.
 *
 * The doc's warning is the thing to watch here: these setters no-op and log
 * when the `threadId` is prop-controlled. This demo therefore never passes a
 * `threadId` prop, so the setters are the single source of truth. The opposite
 * choice is on /headless-threads, which drives the prop instead.
 */
