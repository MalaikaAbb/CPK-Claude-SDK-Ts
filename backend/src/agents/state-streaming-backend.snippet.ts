/**
 * Verbatim from the State Streaming and State Rendering doc pages.
 ...
 *
 * Inert, and for a structural reason worth stating precisely:
 * `emitStreamingDocumentState` consumes RAW Anthropic stream events
 * (`content_block_start`, `content_block_delta` / `input_json_delta`).
 * `ClaudeAgentAdapter` never emits those — it emits AG-UI events
 * (TOOL_CALL_ARGS, STATE_SNAPSHOT, …). The docs call the path that would
 * produce raw deltas "the direct Messages API path" but never publish that
 * run loop, and `WRITE_DOCUMENT_TOOL_SCHEMA` is a backend tool that would
 * additionally need the missing `buildBackendToolServer` bridge.
 *
 * Kept unmodified so the route can show what the docs prescribe next to what
 * the adapter actually does. See README §9.
 */
