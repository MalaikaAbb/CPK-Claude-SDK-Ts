/**
 * Verbatim from the Agent Config doc page.
 * https://docs.copilotkit.ai/claude-sdk-typescript/agent-config
 *
 * Live: the registry uses `AGENT_CONFIG_DEFAULT_SYSTEM_PROMPT` as the agent's
 * base prompt, and `ClaudeAgentAdapter` appends the UI's `useAgentContext`
 * entries underneath it on every run — which is the channel the page is about.
 *
 * `buildAgentConfigSystemPrompt` takes `forwardedProps`, but CopilotKit sends
 * a typed config published with `useAgentContext` as `input.context`, not as
 * forwardedProps. The adapter's `ALLOWED_FORWARDED_PROPS` whitelist also drops
 * unknown keys. So the exported builder is kept as published and the defaults
 * it produces are what the agent starts each run from. See README §9.
 */
